...
extern "C" DLLEXPORT void walkAndPrintVm(uintptr_t pc, uintptr_t sp, uintptr_t fp) {
    ...
    for (int i = 0; i < depth; ++i) {
        method_lookup.resolveMethod(frames[i]);
    }
    ...
    for (int i = 0; i < depth; ++i) {
        ...
        printf("%s::%s\n", classes[mi->_class], strings.at(mi->_name).c_str());
    }
}
